(() => {
  ...
  const TOOL_GROUPS = [
    ...
    { id: "media", label: "媒体", tools: ["gifmaker", "vsplit", "vtrim", "audio", "vplay"] },
    {
      id: "image",
      label: "图片",
      tools: ["imgpreview", "whiteboard", "imgkit", "textimg", "imgtext", "sharecard", "phlogo", "nokiasms", "imgb64", "qrcode"],
    },
  ...
  function syncNavSortDragMode() {
    ...
    if (favoritesList) {
      $$(".nav-fav-link", favoritesList).forEach((link) => {
        link.draggable = allowDrag;
        link.title = allowDrag ? "拖动排序；右键更多操作" : navToolSortHint();
      });
    }
  }
  ...
  function renderFavorites() {
    ...
    items.forEach((id) => {
      ...
      link.title = allowHtml5Drag ? "拖动排序；右键更多操作" : navToolSortHint();
