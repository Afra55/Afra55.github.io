(() => {
  ...
  function start() {
    const panel = document.getElementById("envkit");
    if (!panel || panel.dataset.envkitInit === "1") return;
    panel.dataset.envkitInit = "1";

    const hint = $("#env-run-hint", panel);
    const grid = $("#env-probe-grid", panel);
    const summaryEl = $("#env-probe-summary", panel);
    const errEl = $("#env-error", panel);
    const installBtns = $("#env-install-btns", panel);
    const upgradeBtns = $("#env-upgrade-btns", panel);
    ...
    $("#env-probe", panel).addEventListener("click", () => probeAll());
  ...
  // 面板可能后挂载：先试一次，否则等挂载/路由事件
  if (document.getElementById("envkit")) {
    start();
  } else {
    window.addEventListener("devtools:panel-mounted", (e) => {
      if (!e.detail || e.detail.id === "envkit") start();
    });
    window.addEventListener("devtools:route", (e) => {
      if (!e.detail || e.detail.tool === "envkit") start();
    });
  }
})();
