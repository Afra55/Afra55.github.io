...
- 图片工具：压缩/目标体积、改尺寸、WebP·JPEG·PNG·AVIF、高质量档（mozjpeg/oxipng/AVIF WASM，批量逐张与单张相同）、裁剪、旋转翻转、批量 ZIP、文字/图片水印、圆角边框、EXIF 查看清除、九宫格、App 图标多尺寸、拼接
- ADB 工具：本机桥连接、多设备信息/状态快照、文件管理、APK 安装与信息分析、应用管理（打开/详情/强停/清数据/权限/卸载/备份）、HTTP 代理与端口转发、开发者选项（触摸/布局边界/动画）、性能监控（CPU/内存/FPS）、进程列表与结束、交互 Shell、布局 dump（uiautomator）、Logcat、输入自动化、剪贴板、截图录屏与任务中心（需本机 `adb`；性能/进程/Shell/布局需桥 ≥0.9.0）
- **统一本机桥（17888）**：ADB / FFmpeg(`/ff`) / yt-dlp(`/ytdlp`) / Git(`/git`) 共用一座桥；完整包与 [环境管家](#envkit) 一键装/更。新桥能力必须挂进统一桥并同步 ZIP + EnvKit（见仓库 `AGENTS.md`）。Everything 请用 [外链导航](#sitenav) 打开官网客户端。
