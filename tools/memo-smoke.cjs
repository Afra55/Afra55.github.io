...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    out.cacheBust = {
      version: document.getElementById("site-tools-version")?.textContent || "",
      memoScript: [...document.scripts].some((s) => /memo\.js\?v=20260817vsplitnudge2/.test(s.src)),
    };
  ...
  if (!/memo|theme|vsplit|vtrim|audio|ffb|ffadapt|setup|btnsize|memoux|imgzoom|vsfsjank|vsplitfs|pwa|navpwa|memopin|navfoot|navlast|navfly/i.test(result.version)) failed.push(`unexpected version ${result.version}`);
  ...
  if (!/vsplitnudge2/i.test(result.cacheBust?.version || "") || !result.cacheBust?.memoScript) {
    failed.push("cache-bust/version should be aligned to vsplitnudge2");
  }
