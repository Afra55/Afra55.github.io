...
async function main() {
  ...
  if (!/navctx1/i.test(result.version)) failed.push(`unexpected version ${result.version}`);
  ...
  if (!/navctx1/i.test(result.cacheBust?.version || "")) {
    failed.push("cache-bust/version should be aligned to navctx1");
  }
