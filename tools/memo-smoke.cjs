...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    out.cacheBust = {
      ...
      version: document.getElementById("site-tools-version")?.textContent || "",
      memoScript: [...document.scripts].some((s) => /memo\.js\?v=20260816scrcpy1/.test(s.src)),
    };
  ...
  if (!/scrcpy1/i.test(result.cacheBust?.version || "") || !result.cacheBust?.memoScript) {
    failed.push("cache-bust/version should be aligned to scrcpy1");
  }
