...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    out.cacheBust = {
      ...
      version: document.getElementById("site-tools-version")?.textContent || "",
      memoScript: [...document.scripts].some((s) => /memo\.js\?v=20260816vtrim3/.test(s.src)),
    };
  ...
  if (!/memo|theme|vsplit|vtrim/i.test(result.version)) failed.push(`unexpected version ${result.version}`);
  ...
  if (!/vtrim3/i.test(result.cacheBust?.version || "") || !result.cacheBust?.memoScript) {
    failed.push("cache-bust/version should be aligned to vtrim3");
  }
