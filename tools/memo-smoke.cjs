...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    out.cacheBust = {
      ...
      version: document.getElementById("site-tools-version")?.textContent || "",
      memoScript: [...document.scripts].some((s) => /memo\.js\?v=20260816vsplit1/.test(s.src)),
    };
  ...
  if (!/memo|theme|vsplit/i.test(result.version)) failed.push(`unexpected version ${result.version}`);
  ...
  if (!/vsplit1/i.test(result.cacheBust?.version || "") || !result.cacheBust?.memoScript) {
    failed.push("cache-bust/version should be aligned to vsplit1");
  }
