...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    out.cacheBust = {
      ...
      version: document.getElementById("site-tools-version")?.textContent || "",
      memoScript: [...document.scripts].some((s) => /memo\.js\?v=20260816vsplit3/.test(s.src)),
    };
  ...
  if (!/vsplit3/i.test(result.cacheBust?.version || "") || !result.cacheBust?.memoScript) {
    failed.push("cache-bust/version should be aligned to vsplit3");
  }
